...
int main()
{
    ...
    int n = 0;
    int temp;
    //Coloca cada linha do arquivo em uma posi��o do vetor linha[]
    ...
    //----------------------------------------
    //Declara��o de tabelas de simbolos e de definicao
    ...
    //n � o tamanho de linha[] (qtd de linhas no arquivo)
    for (i = 0; i < n; i++)
    { // Obt�m uma linha do fonte
        ch = strtok(linha[i], " ");
        //printf("Contador Linha: %d\n", contador_linha);
        contador_linha++;
        // AQUI COMECA A SEPARAR A LINHA (ch):
        while (ch != NULL)
        { // Separa os elementos da linha: r�tulo, opera��o, operandos, coment�rios
            //printf("Contador Posicao: %d\n", contador_posicao);
            //printf("%s\n", ch);   // TA IMPRIMINDO CADA PALAVRA SEPARADA JA AQUI
            if (testaComentario(ch) == 1)
            { // Ignora os coment�rios
                ch = strtok(NULL, " \n");
                break;
            }
            if (testaRotulo(ch) == 1)
            { // SE FOR R�TULO (acaba em :), guarda o contador_posicao    // Se existe r�tulo:
                //printf("rot: %s\n", retornaRotuloSemDoisPontos(ch));
                for (j = 0; j < n; j++)
                { // Procura r�tulo na TS (Tabela de S�mbolos)
                    if (strcmp(retornaRotuloSemDoisPontos(ch), tabela_simbolos[j].rotulo) == 0)
                    { // Se achou: Erro --> s�mbolo redefinido
                        printf("ERRO -> simbolo redefinido! TEM QUE SAIR DO PROGRAMA?\n");
                        break;
                    }
                }
                strcpy(tabela_simbolos[i].rotulo, retornaRotuloSemDoisPontos(ch));
                tabela_simbolos[i].endereco = contador_posicao; // Sen�o: Insere r�tulo e contador_posi��o na TS
            }

            if (testaOpcode(ch, mne) > 0)
            { // Achou o opcode                                     // Procura opera��o na tabela de instru��es
                printf("%d", contador_posicao);
                contador_posicao++; // Se achou: contador_posi��o = contador_posi��o + tamanho da instru��o
                printf("-%s- ", ch);
                cod = testaOpcode(ch, mne);
                printf("(cod %d, %d operandos)\n", cod + 1, instrucoes[cod + 1].qtdOperandos);
                contador_posicao = contador_posicao + instrucoes[cod + 1].qtdOperandos; // j� achou a instrucao, entao pula a linha
                break;
            }
            else
            { // N�o achou o opcode                                   // Sen�o:
                // Procura opera��o na tabela de diretivas
                if (testaDiretiva(ch, mneDir) >= 0)
                { // achou a diretiva, chama a subrotina adequada
                    ...
                    case 1: // SPACE - Reservar 1 ou mais enderecos de memoria nao-inicializada para armazenamento de uma palavra. ok
                        ch = strtok(NULL, " \n");
                        if (ch != NULL)
                        { // ou seja, eh um vetor que esta sendo reservado
                            //printf("TESTE: -%s-\n", ch);
                            //printf("%d ", contador_posicao);
                ...
                { // nao achou a diretiva --> Erro, opera��o n�o identificada
